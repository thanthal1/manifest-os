...
/// the application menu / launcher (fuzzel, rofi, …), launching through the shim.
/// Only binaries that actually ship a `.desktop` get an entry — CLI tools (htop,
/// git, apt) don't, so the menu isn't cluttered. A `.desktop` matches an exposed
/// binary either by **filename** (`chromium` → `chromium.desktop`) or by its
/// **`Exec` program** (`org.mozilla.firefox.desktop` whose `Exec` runs `firefox`)
/// — so reverse-DNS- and wrapper-named entries still surface in the launcher.
fn write_desktop_entries(s: &Stratum, ctx: &Ctx) -> Result<()> {
    let root = stratum_root(&s.name);
    let appdir = format!("{root}/usr/share/applications");
    // Read the stratum's .desktop files once (filename, content).
    let desktops: Vec<(String, String)> = std::fs::read_dir(&appdir)
        .into_iter()
        .flatten()
        .flatten()
    ...
    let mut written: std::collections::HashSet<String> = Default::default();
    for bin in &s.expose {
        if runs_as_root(bin) {
            continue; // package managers aren't GUI apps
        }
        for (name, content) in &desktops {
            let matches = name == &format!("{bin}.desktop")
                || desktop_exec_bin(content).as_deref() == Some(bin.as_str());
            if !matches || written.contains(name) {
                continue; // no match, or already mirrored under another bin
            }
            let entry = rewrite_desktop(content, &shim_path(bin));
            let dest = format!("{APPLICATIONS_DIR}/strata-{}-{name}", s.name);
...
/// The program a `.desktop` launches: the basename of the first token of its
/// first `Exec=` line (field codes like `%U` and any path stripped). Lets us
/// match a `.desktop` to an exposed binary when the filename isn't `<bin>.desktop`.
fn desktop_exec_bin(content: &str) -> Option<String> {
    let rest = content.lines().find_map(|l| l.strip_prefix("Exec="))?;
...
mod tests {
    ...
    #[test]
    fn desktop_exec_bin_extracts_program_basename() {
        // Plain path + field code.
        assert_eq!(
            desktop_exec_bin("[Desktop Entry]\nExec=/usr/bin/firefox %u\n").as_deref(),
            Some("firefox")
        );
        // Bare name, no path.
        assert_eq!(desktop_exec_bin("Exec=foot\n").as_deref(), Some("foot"));
        // First Exec wins (top-level, not a Desktop Action further down).
        assert_eq!(
            desktop_exec_bin("Name=X\nExec=/usr/bin/chromium %U\n[Action]\nExec=/usr/bin/other\n")
                .as_deref(),
            Some("chromium")
        );
        // No Exec at all.
        assert_eq!(desktop_exec_bin("[Desktop Entry]\nName=X\n"), None);
    }
